...
const ChatContent = ({ content }: { content?: string }) => {
  ...
  return (
    <div className="size-full relative bottom-0">
      <motion.div
        ref={ref}
      ...
      >
        ...
        <div className="absolute bottom-0 w-full flex justify-center items-center bg-background border-t border-gray-400 p-4">
...
export const ChatFeedBack = ({ feedback }: ChatFeedbackProps) => {
  const { close, isOpen } = useControlFeedBack()
  return (
    <>
      <FadeInOutWrapper isVisible={isOpen}>
        <div
          className="absolute inset-0 z-50 size-full bg-foreground/80"
          onClick={close}
        >
